...
private:
    ...
    int rescue_trigger_failures;  // 触发救援的连续 failed 次数阈值
    int rescue_trigger_stuck_abort_count;  // 触发救援的 stuck/abort 累计次数阈值
...
public:
    FrontierGoalManager() : pnh("~"), ac("move_base", true), gen(std::random_device{}())
    {
        ...
        pnh.param("rescue_trigger_failures", rescue_trigger_failures, 5);
        pnh.param("rescue_trigger_stuck_abort_count", rescue_trigger_stuck_abort_count, 10);
        pnh.param("rescue_goal_timeout", rescue_goal_timeout, 15.0);
        ...
        ROS_INFO("  - rescue_trigger_stuck_abort_count: %d (stuck/abort counter)", rescue_trigger_stuck_abort_count);
    ...
    void markGoalAsFailed(double x, double y, double cool_down_multiplier = 1.0)
    {
        failed_frontiers.emplace_back(
            ...
            y,
            ros::Time::now(),
            ros::Time::now() + ros::Duration(failed_frontier_cool_down * cool_down_multiplier)
        );
    }
    ...
    bool shrinkOnGlobalCostmap(double &target_x, double &target_y, double robot_x, double robot_y)
    {
        ...
        int mx = static_cast<int>(std::round((target_x - map_origin_x) / res));
        int my = static_cast<int>(std::round((target_y - map_origin_y) / res));
        ...
        while (current_shrink_dist < global_shrink_radius) {
            ...
            int n_mx = static_cast<int>(std::round((new_x - map_origin_x) / res));
            int n_my = static_cast<int>(std::round((new_y - map_origin_y) / res));
            ...
            if (n_is_safe) {     
                ...
                while (extra_dist < global_extra_shrink_distance) {
                    ...
                    int e_mx = static_cast<int>(std::round((extra_x - map_origin_x) / res));
                    int e_my = static_cast<int>(std::round((extra_y - map_origin_y) / res));
    ...
    bool checkNearbyObstacleRunning()
    {
        ...
        MapCoords coords = worldToMapCoords(tx, ty, global_costmap);
        if (!coords.valid) return false;

        int8_t raw = global_costmap.data[coords.my * global_costmap.info.width + coords.mx];
    ...
    bool isNearbyCostSafe(double x, double y)
    {
        ...
        MapCoords coords = worldToMapCoords(x, y, global_costmap);
        if (!coords.valid) return false;
    ...
    double calculateUnknownProportion(double x, double y) {
        ...
        int cx = static_cast<int>(std::round((x - origin_x) / res));
        int cy = static_cast<int>(std::round((y - origin_y) / res));

        if (cx < 0 || cx >= map.info.width || cy < 0 || cy >= map.info.height) {
            return 0.0;
        }

        // 使用 ceil 保证搜索半径不被截断缩小
        int radius_cells = std::max(1, static_cast<int>(std::ceil(known_area_search_radius / res)));
    ...
    double calculateFrontierOrientation(double x, double y, double rx, double ry)
    {
        ...
        int radius_cells = std::max(1, static_cast<int>(std::ceil(orientation_search_radius / res)));
        radius_cells = std::min(radius_cells, 50); 
        int min_x = std::max(0, cx - radius_cells);
    ...
    FrontierCluster selectGoal(bool force_revive)
    {
        ...
        for (auto& c : clusters)
        {
            ...
            if (!shrinkOnGlobalCostmap(c.x, c.y, rx, ry)) { shrink_fail++; continue; }
            if (!isNearbyCostSafe(c.x, c.y)) { obstacle_fail++; continue; }

            bool blacklisted = false;
            for (const auto& f : failed_frontiers) {
                if (hypot(c.x - f.x, c.y - f.y) < failed_frontier_tolerance) { blacklisted = true; break; }
            }
            if (!force_revive && blacklisted) { blacklist_fail++; continue; }

            valid_clusters.push_back(&c);
            valid_count++;
        }

        // 【修复】在 valid cluster 中重新计算 max_num
        int max_num = 1;
        for (const auto* c : valid_clusters) if(c->num > max_num) max_num = c->num;

        // 【修复】第二遍：用正确的 max_num 计算权重
        for (auto* c : valid_clusters)
        {
            double rx, ry, ryaw;
            getRobotPose(rx, ry, ryaw);
            c->distance = hypot(c->x - rx, c->y - ry);
    ...
    void finalizeCancel()
    {
        ROS_INFO("Finalizing Cancel. consecutive_failures=%d, is_rescue_mode=%d", consecutive_plan_failures, is_rescue_mode);
        ...
        // 【修复】不再在这里重置救援模式状态，只在救援成功或达到最大尝试次数时重置
        // 之前的逻辑导致刚进入救援模式就被前一个 goal 的 cancel 确认退出
        ...
        if (state == actionlib::SimpleClientGoalState::ABORTED ||
            ...
            if (is_rescue_mode) {
                ...
                ROS_INFO("[Rescue Counter] stuck_abort_count = %d/%d (move_base ABORTED)",
                         stuck_abort_count, rescue_trigger_stuck_abort_count);
                ...
                if (distance_initialized && (ros::Time::now()-last_distance_time).toSec() > goal_check_interval) {
                    if (last_distance - dist < distance_change_threshold) {
                        ...
                        if(slow_count >= slow_count_limit) {
                            ...
                            // 【修复】stuck 时也计入 stuck_abort_count，但第一次要对比位移
                            double rx, ry, ryaw;
                            if (getRobotPose(rx, ry, ryaw)) {
                                if (!last_stuck_abort_check_time.isValid()) {
                                    // 第一轮第一次：记录位置，不计数
                                    ...
                                    last_robot_position.y = ry;
                                    last_stuck_abort_check_time = ros::Time::now();
                                    ROS_INFO("[Stuck] First stuck detected, recording position (%.2f, %.2f)", rx, ry);
                                } else {
                                    // 后续：对比位移，超过 0.5m 则清零计数
                                    double displacement = std::hypot(rx - last_robot_position.x, ry - last_robot_position.y);
                                    if (displacement > 0.5) {
                                        ROS_INFO("[Rescue Counter] Robot moved %.2fm during stuck, reset stuck_abort_count to 0", displacement);
                                        stuck_abort_count = 0;
                                    } else {
                                        // 位移小于 0.5m，计入 stuck_abort_count
                                        stuck_abort_count++;
                                        ROS_INFO("[Rescue Counter] stuck_abort_count = %d/%d (Robot Stuck)", 
                                                 stuck_abort_count, rescue_trigger_stuck_abort_count);
                                    }
                                    // 更新位置
                                    last_robot_position.x = rx;
        ...
        if(state == actionlib::SimpleClientGoalState::SUCCEEDED) {
            ...
            if (!is_rescue_mode) {
                markGoalAsFailed(
                    current_goal.target_pose.pose.position.x,
                    current_goal.target_pose.pose.position.y,
                    2.0  // 成功时也加入黑名单，但时间是 2 倍 cool_down
                );
            }
            goal_active = false;
            plan_received = false;
    ...
    void spin()
    {
        ...
        while(ros::ok()) {
            ...
            if (!is_rescue_mode && stuck_abort_count >= rescue_trigger_stuck_abort_count) {

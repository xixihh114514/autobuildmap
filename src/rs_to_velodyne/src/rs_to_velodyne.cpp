...
void publish_points(T &new_pc, const sensor_msgs::PointCloud2 &old_msg) {
    ...
    pc_new_msg.header.stamp = ros::Time::now(); // 使用当前系统时间
...
void handle_pc_msg(const typename pcl::PointCloud<T_in_p>::Ptr &pc_in,
    ...
    for (int point_id = 0; point_id < pc_in->points.size(); ++point_id) {
        if (has_nan(pc_in->points[point_id]))
            continue;
        T_out_p new_point;
        std::copy(pc_in->points[point_id].data, pc_in->points[point_id].data + 4, new_point.data);
...
void rsHandler_XYZIRT(const sensor_msgs::PointCloud2 &pc_msg) {
    ...
    */
    ...
    // 转换点云
    for (const auto& point : pc_in->points) {
        // 跳过包含 nan 的点
        if (has_nan(point)) {
            continue;
        }
        ...
        new_point.time = static_cast<float>(point.timestamp - start_time); // 转换为相对时间（秒）
        pc_out->points.push_back(new_point);
    }

    // 设置点云属性
    pc_out->is_dense = true; // 已过滤 nan 点，因此是密集点云
    pc_out->height = 1;      // Velodyne 点云通常是无组织点云
    pc_out->width = pc_out->points.size();

    // 发布点云
    publish_points(pc_out, pc_msg);

    //ROS_INFO("Input points: %zu, Output points: %zu", pc_in->points.size(), pc_out->points.size());
}
...
int main(int argc, char **argv) {
    ...
    */
    ...
    // 从参数服务器读取输入和输出点云格式
    nh.param<std::string>("input_type", input_type, "XYZIRT");
    nh.param<std::string>("output_type", output_type, "XYZIRT");

    if (input_type != "XYZI" && input_type != "XYZIRT") {
        ROS_ERROR("Unsupported input pointcloud type. Currently only support XYZI and XYZIRT.");
        exit(1);
    }

    if (output_type != "XYZI" && output_type != "XYZIR" && output_type != "XYZIRT") {
        ROS_ERROR("Unsupported output pointcloud type. Currently only support XYZI, XYZIR, and XYZIRT.");
        exit(1);
    }

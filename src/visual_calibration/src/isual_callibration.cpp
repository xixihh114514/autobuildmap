...
class VisualCalibrationNode {
 public:
  ...
  VisualCalibrationNode()
    ...
    detection_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(detection_cloud_topic_, 1);
 ...
 private:
  ...
  void workerLoop() {
    while (ros::ok() && running_.load()) {
      ...
      publishDetectionCloud(detections);
  ...
  cv::Point3f transformPointToDepthFrame(const cv::Point3f& point,
                                         const std::string& source_frame) const {
    if (source_frame == detection_cloud_frame_) {
      return point;
    }

    if (source_frame == "camera_depth_optical_frame" && detection_cloud_frame_ == "camera_depth_frame") {
      return cv::Point3f(point.z, -point.x, -point.y);
    }

    if (source_frame == "camera_depth_frame" && detection_cloud_frame_ == "camera_depth_optical_frame") {
      return cv::Point3f(-point.y, -point.z, point.x);
    }
  ...
  void publishDetectionCloud(const std::vector<Detection>& detections) {
    if (detection_cloud_pub_.getNumSubscribers() == 0) {
      return;
    }
    ...
    for (const auto& detection : detections) {
      ...
      const std::string source_frame =
          cloud ? cloud->header.frame_id : std::string("camera_depth_optical_frame");
      points.push_back(transformPointToDepthFrame(detection.position, source_frame));
    }

    sensor_msgs::PointCloud2 out;
    out.header.stamp = cloud ? cloud->header.stamp : ros::Time::now();
    ...
    for (size_t i = 0; i < points.size(); ++i) {
      ...
      std::memcpy(&out.data[offset + 0], &points[i].x, sizeof(float));
      std::memcpy(&out.data[offset + 4], &points[i].y, sizeof(float));
      std::memcpy(&out.data[offset + 8], &points[i].z, sizeof(float));
    }
